...
setClass(Class = "sysBiolAlg_FastGlobalFit_MULT_BIOM",
         representation(
             wu  = "numeric",
             wl  = "numeric",
             fnc = "integer",
...
# contructor for class sysBiolAlg_FastGlobalFit_MULT_BIOM
setMethod(f = "initialize",
          signature = "sysBiolAlg_FastGlobalFit_MULT_BIOM",
		...
		{	
			...
			for(i in 1:length(off))
			{
				...
				if(!is.null(off[[i]]$biomass))
				{
					...
					{
						pos=which(react_id(model_temp)==bm[k])
						if(length(pos)>0)
						{
							biomass_pos=pos
							vec=obj_coef(model_temp)
							vec[biomass_pos]=1
							obj_coef(model_temp)=vec
						}
					}
				...
				deleteM[(nr+bio_pos),(1+3*nc+nr+1+length(remove_biomass_metabolites)*3)]=0
				
				
				
				deleteM[(nr+nc+1+length(remove_biomass_metabolites)*1),(1+3*nc+nr+1+length(remove_biomass_metabolites)*3)]=0
...
#
#                  .Object@problem   <- lp
#                  .Object@algorithm <- "sysBiolAlg_FastGlobalFit_MULT_BIOM"
